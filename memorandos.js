...
var Memorandos = (function() {
  ...
  function classificar(m) {
    ...
    if (m.criado_por === u.id) return 2;
    if (m.data_envio_servicos && APP.isDds()) return 4;
    if (m.aprovado_dds && (APP.isDds() || m.criado_por === u.id)) return 3;
    if (!m.criado_por_dds && m.criado_por !== u.id && APP.isDds()) return 1;
    return 0;
  }
  ...
  function criar() {
    ...
    gerarNumMemo().then(function(num) {
      ...
      toast('Erro ao criar. Verifique o Supabase.', 'error');
    });
  }
