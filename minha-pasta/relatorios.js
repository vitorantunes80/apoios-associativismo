...
var Relatorios = (function() {
  ...
  function _indicadoresDisp(eq) {
    var l = [];
    if (modAtivo(eq, 'estadias')) {
      l.push({ campo: 'hospedes', label: 'Hóspedes',   cor: CORES.teal });
      l.push({ campo: 'estadias', label: 'Estadias',   cor: CORES.azul });
      l.push({ campo: 'dormidas', label: 'Dormidas',   cor: CORES.verde });
    }
    if (modAtivo(eq, 'visitas')) {
      l.push({ campo: 'visitantes', label: 'Visitantes', cor: CORES.roxo });
      l.push({ campo: 'visitas',    label: 'Grupos',     cor: CORES.azul });
    }
    if (modAtivo(eq, 'reservas')) l.push({ campo: 'reservas', label: 'Reservas', cor: CORES.azul });
    if (!l.length) l.push({ campo: 'memosCo', label: 'Reparações concluídas', cor: CORES.verde });
    return l;
  }
