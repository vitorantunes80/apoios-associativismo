...
var Relatorios = (function() {
  ...
  function render() {
    ...
    [['indicadores', 'Indicadores'], ['horas', 'Banco de Horas'], ['impressao', 'Imprimir / PDF']].forEach(function(t) {
  ...
  function _renderResumoTopo() {
    ...
    h += _repCard(mAbertos, 'Memorandos em aberto', 'de ' + M.length + ' no total', '#D97706', '#FEF3C7');
    h += _repCard(taxa + '%', 'Taxa de conclusão', 'memorandos concluídos', '#7C3AED', '#EDE9FE');
    h += '</div>';
    return h;
  }
  ...
  function _renderHorasIndicadores() {
    ...
    h += _repCard(Horas.fmtH(tEf), 'Horas efectuadas', uni ? ('Unidade ' + uni) : 'Toda a DDS', '#15803D', '#DCFCE7');
    h += _repCard(Horas.fmtH(tGo), 'Horas gozadas', uni ? ('Unidade ' + uni) : 'Toda a DDS', '#D97706', '#FEF3C7');
    h += _repCard(Horas.fmtH(tEf - tGo), 'Saldo em dívida', 'a compensar aos funcionários', (tEf - tGo) > 0 ? '#B91C1C' : '#15803D', '#FEE2E2');
    h += _repCard(nPend, 'Por validar', 'registos pendentes', '#2563EB', '#DBEAFE');
    h += '</div>';

    // saldos por funcionário
    var linhas = funcs.map(function(f) {
      var s = Horas.saldoDe(f.id);
      return { f: f, s: s };
    }).filter(function(x) {
    ...
    h += '<div class="sec-head"><div class="sec-title" style="font-size:14px;">Saldos por funcionário</div>';
    h += '<button class="btn btn-secondary btn-sm" onclick="Relatorios.imprimir(\'horas\',\'' + (uni || '') + '\')">🖨️ Imprimir</button></div>';

    if (!linhas.length) {
      h += emptyState('Sem saldos', 'Nenhum funcionário tem horas aprovadas.');
    } else {
      h += '<div class="dtw"><table class="dt">';
      h += '<thead><tr><th>Funcionário</th><th>Unidade</th><th>Efectuadas</th><th>Gozadas</th><th>Saldo</th></tr></thead><tbody>';
      linhas.forEach(function(x) {
        ...
        h += '<td class="td-m">' + Horas.fmtH(x.s.efetuadas) + '</td>';
        h += '<td class="td-m">' + Horas.fmtH(x.s.gozadas) + '</td>';
  ...
  function _renderListaFuncionarios() {
    ...
    // Banco de horas
    h += '<div class="rep-grp"><div class="rep-grp-t">Banco de Horas (saldos e movimentos)</div>';
  ...
  function _repHoras(unidade) {
    ...
    if (movs.length) {
      h += '<div class="pr-sec">Movimentos aprovados (' + movs.length + ')</div>';
      h += '<table class="pr-table">';
  ...
  return {
    render: render, init: init, setTab: setTab,
    ...
    imprimirEq: imprimirEq, imprimirEqSel: imprimirEqSel
  };
