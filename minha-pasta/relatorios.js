...
var Relatorios = (function() {
  ...
  function _chartBarras(dados, cor) {
    var W = 380, H = 150, pb = 22, pt = 16, pl = 6;
    var alt = H - pt - pb;
    var max = 1;
    ...
    dados.forEach(function(d, i) {
      ...
      if (d.v > 0) {
        s += '<text x="' + (x + bw / 2).toFixed(1) + '" y="' + (y - 4).toFixed(1) +
             '" text-anchor="middle" font-size="8.5" font-weight="700" fill="#334155">' + d.v + '</text>';
      }
      s += '<text x="' + (x + bw / 2).toFixed(1) + '" y="' + (H - 7) +
           '" text-anchor="middle" font-size="8.5" fill="#64748B">' + esc(d.l) + '</text>';
    });
    s += '</svg>';
    return s;
  }
  ...
  function _renderEquipamentos() {
    ...
    // ── Comparação entre equipamentos (só DDS) ──
    if (APP.isDds() && eqs.length > 1) {
      var comp = eqs.map(function(e) {
        var de = _indicadores(e.id, mes);
        return { l: e.nome, v: de[ind.campo] || 0 };
      }).sort(function(a, b) { return b.v - a.v; });
      html += _bloco('Comparação entre equipamentos — ' + ind.label,
                     _chartBarrasH(comp, ind.cor), esc(_perLabel()));
    }
  ...
  // Qual o indicador que melhor descreve este equipamento
  function _indicadorPrincipal(eq) {
    if (modAtivo(eq, 'estadias')) return { campo: 'dormidas',   label: 'Dormidas',   cor: CORES.teal };
    if (modAtivo(eq, 'visitas'))  return { campo: 'visitantes', label: 'Visitantes', cor: CORES.roxo };
    if (modAtivo(eq, 'reservas')) return { campo: 'reservas',   label: 'Reservas',   cor: CORES.azul };
    return { campo: 'memosCo', label: 'Reparações concluídas', cor: CORES.verde };
  }
  ...
  function _repEqMensal(eq) {
    ...
    // Gráfico de evolução — o SVG imprime como qualquer imagem
    var serie = [];
    for (var m = 1; m <= 12; m++) {
      serie.push({ l: MESES_C[m - 1], v: _indicadores(eq.id, m)[ind.campo] || 0, destaque: (mes === m) });
    }
    h += '<div class="pr-sec">Evolução mensal — ' + esc(ind.label) + '</div>';
    h += '<div class="pr-chart">' + _chartBarras(serie, ind.cor) + '</div>';

    // Detalhe das estadias
    if (modAtivo(eq, 'estadias') && d.estadiasL.length) {
      h += '<div class="pr-sec">Estadias (' + d.estadiasL.length + ')</div>';
      h += '<table class="pr-table"><thead><tr><th>Entrada</th><th>Saída</th><th>Adultos</th><th>Crianças</th><th>Noites</th><th>Dormidas</th></tr></thead><tbody>';
      d.estadiasL.forEach(function(e) {
        h += '<tr><td>' + _fdataC(e.data_checkin) + '</td>' +
             '<td>' + _fdataC(e.data_checkout_real || e.data_checkout_prev) + '</td>' +
             '<td>' + (e.n_adultos || 0) + '</td><td>' + (e.n_criancas || 0) + '</td>' +
             '<td>' + _noitesEst(e) + '</td><td>' + _dormidasEst(e) + '</td></tr>';
      });
      h += '<tr class="pr-tot"><td colspan="5">Total de dormidas</td><td>' + d.dormidas + '</td></tr>';
      h += '</tbody></table>';
    }

    // Detalhe das visitas
    if (modAtivo(eq, 'visitas') && d.visitasL.length) {
      h += '<div class="pr-sec">Visitas (' + d.visitasL.length + ')</div>';
      h += '<table class="pr-table"><thead><tr><th>Data</th><th>Tipo</th><th>Entidade</th><th>Adultos</th><th>Crianças</th><th>Total</th></tr></thead><tbody>';
      d.visitasL.forEach(function(v) {
        h += '<tr><td>' + _fdataC(v.data) + '</td><td>' + esc(_labelTipoVisita(v.tipo_visitante)) + '</td>' +
             '<td>' + esc(v.entidade || '—') + '</td><td>' + (v.n_adultos || 0) + '</td>' +
             '<td>' + (v.n_criancas || 0) + '</td><td>' + _pessoasReg(v) + '</td></tr>';
      });
      h += '<tr class="pr-tot"><td colspan="5">Total de visitantes</td><td>' + d.visitantes + '</td></tr>';
      h += '</tbody></table>';
    }
    ...
    if (memosP.length) {
      h += '<div class="pr-sec">Reparações e memorandos (' + memosP.length + ')</div>';
      h += '<table class="pr-table"><thead><tr><th>Data</th><th>Assunto</th><th>Estado</th></tr></thead><tbody>';
      memosP.forEach(function(m) {
    ...
    h += '<div style="margin-top:26px;display:flex;gap:40px;font-size:9px;">';
    h += '<div style="flex:1;border-top:1px solid #999;padding-top:4px;">O responsável do equipamento</div>';
    h += '<div style="flex:1;border-top:1px solid #999;padding-top:4px;">O chefe de unidade</div>';
    h += '</div>';
  ...
  return {
    ...
    setEqRel: setEqRel, recarregar: recarregar, imprimirMensal: imprimirMensal
  };
