...
var Relatorios = (function() {
  ...
  function _renderEquipamentos() {
    ...
    // ── Comparação — só entre equipamentos do MESMO TIPO ──
    // Comparar um parque de campismo com um pavilhão não tem significado:
    // as utilizações e as unidades de medida são diferentes.
    if (APP.isDds()) {
      var iguais = eqs.filter(function(e) { return e.tipo_id === eq.tipo_id; });
      if (iguais.length > 1) {
        var comp = iguais.map(function(e) {
          var de = _indicadores(e.id, mes);
          return { l: e.nome, v: de[ind.campo] || 0 };
        }).sort(function(a, b) { return b.v - a.v; });
        html += _bloco('Comparação — ' + ind.label,
                       _chartBarrasH(comp, ind.cor),
                       (tipo ? tipo.nome : 'Mesmo tipo') + ' · ' + _perLabel());
      }
    }
  ...
  function _repEqMensal(eq) {
    ...
    // ── Gráficos de evolução: UM POR INDICADOR ──
    // (a lista linha a linha das estadias/visitas ocupava dezenas de páginas
    //  e não acrescentava nada a um relatório mensal de gestão)
    var disp = _indicadoresDisp(eq);
    disp.forEach(function(i) {
      var serie = [];
      for (var m = 1; m <= 12; m++) {
      ...
      h += '<div class="pr-sec">Evolução mensal — ' + esc(i.label) + '</div>';
      h += '<div class="pr-chart">' + _chartBarras(serie, i.cor) + '</div>';
    });

    // Repartição (donut) — dá contexto sem alongar o documento
    if (modAtivo(eq, 'visitas') && d.visitas) {
      var porTipo = {};
      d.visitasL.forEach(function(v) {
        var t = v.tipo_visitante || 'outro';
      ...
      h += '<div class="pr-sec">Visitantes por tipo</div>';
      h += '<div class="pr-chart">' + _chartDonut(dd) + '</div>';
    }
    ...
    if (modAtivo(eq, 'estadias') && d.estadias) {
      ...
      h += '<div class="pr-sec">Hóspedes por escalão</div>';
      h += '<div class="pr-chart">' +
           _chartDonut([{ l: 'Adultos', v: ad, c: CORES.azul }, { l: 'Crianças', v: cr, c: CORES.ambar }]) +
           '</div>';
    }
