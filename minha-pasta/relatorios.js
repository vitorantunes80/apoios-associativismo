...
var Relatorios = (function() {
  ...
  function _renderListaFuncionarios() {
    ...
    // Equipamentos
    h += '<div class="rep-grp"><div class="rep-grp-t">Equipamentos (' + nEq + ')</div>';
  ...
  function _repEquipamentos(unidade) {
    ...
    h += '<table class="pr-table"><thead><tr>';
    ['#', 'Equipamento', 'Unidade', 'Responsável', 'Estado', 'Capac.'].forEach(function(c){ h += '<th>' + c + '</th>'; });
    h += '</tr></thead><tbody>';
    lista.forEach(function(e, i) {
      ...
      h += '<tr><td>' + (i + 1) + '</td><td>' + esc(e.nome) + '</td><td>' + esc(e.unidade || '—') + '</td><td>' + esc(resp || '—') + '</td><td>' + esc(estadoLbl[e.estado] || e.estado || '—') + '</td><td>' + esc(e.capacidade || '—') + '</td></tr>';
    });
    h += '</tbody></table>' + _rodape();
    return h;
  }
