...
var Checklist = (function() {
  ...
  function render(eqId) {
    ...
    if (!eq) return emptyState('Sem equipamento', 'Seleccione um equipamento.');

    var tipo = getTipo(eq.tipo_id);
    if (tipo && !tipo.mod_checklist) {
      return '<div class="mod-ph"><div class="mod-ph-t">Módulo não activo</div><div class="mod-ph-s">Checklists não activadas para este tipo de equipamento.</div></div>';
    }

    var html = '<div class="dash-greeting">';
    html += '<div><div class="dash-hello" style="font-size:18px;">Checklists — ' + H(eq.nome) + '</div></div>';
    html += '<div style="display:flex;gap:8px;">';
    if (APP.isAdmin() || APP.isDds()) html += '<button class="btn btn-secondary btn-sm" onclick="Checklist.openGerirTemplates(\'' + eq.id + '\')">⚙ Gerir templates</button>';
