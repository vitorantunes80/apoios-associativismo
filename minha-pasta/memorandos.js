...
var Memorandos = (function() {
  ...
  function classificar(m) {
    ...
    // sec2 — Os Meus: criador acompanha memorandos ainda não enviados
    // Uma vez enviado, passa para sec4 (Em Curso) — visível na DDS
    if (euCriei && !m.data_envio_servicos) return 2;

    // DDS vê os fluxos completos
    if (APP.isDds()) {
      if (m.data_envio_servicos) return 4;    // Em Curso (inclui os do próprio criador)
      if (m.aprovado_dds)        return 3;    // A Enviar
      if (euCriei)               return 2;    // Os Meus (pendente, não aprovado)
      if (!m.criado_por_dds)     return 1;    // Recebidos de equipamento
    }

    // não-DDS: se criou e não foi enviado, vê em Os Meus
    if (euCriei) return 2;
