...
var Memorandos = (function() {
  ...
  function _renderSec3() {
    ...
    // acções: copiar o texto pronto a colar no email
    html += '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:14px;">';
    html += '<button class="btn btn-primary btn-sm" onclick="Memorandos.copiarEmail()">📋 Copiar para email (' + memos.length + ')</button>';
    html += '<button class="btn btn-secondary btn-sm" onclick="Memorandos.preverEmail()">Pré-visualizar</button>';
    html += '<span style="font-size:11.5px;color:var(--text-3);">Copia todos os memorandos em texto, pronto a colar.</span>';
    html += '</div>';
  ...
  function _textoEmail() {
    ...
    Object.keys(porBrig).sort().forEach(function(b) {
      ...
      porBrig[b].forEach(function(m) {
        ...
        L.push('Memorando n.º ' + (m.numero || '—') + '  |  ' + fData(m.data));
  ...
  function copiarEmail() {
    ...
    // método moderno; com recurso a alternativa se falhar (Safari exige gesto do utilizador)
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(txt).then(function() {
        toast(memos.length + ' memorando(s) copiados. Cole no email.', 'success');
      }).catch(function() {
        _copiarFallback(txt, memos.length);
      });
    } else {
      _copiarFallback(txt, memos.length);
    }
  ...
  // alternativa: selecciona o texto numa textarea para o utilizador copiar à mão
  function _copiarFallback(txt, n) {
    ...
    if (ok) {
      toast(n + ' memorando(s) copiados. Cole no email.', 'success');
    } else {
  ...
  function getModaisHTML() {
    ...
    html += '<p style="font-size:12.5px;color:var(--text-2);line-height:1.6;margin:0 0 10px;">Seleccione tudo e copie, ou use o botão em baixo.</p>';
    html += '<textarea id="m-email-txt" class="fin" rows="16" style="font-family:ui-monospace,Menlo,monospace;font-size:12px;line-height:1.55;white-space:pre;" onclick="this.select()"></textarea>';
    html += '</div>';
    html += '<div class="modal-footer"><button class="btn btn-ghost" onclick="closeModal(\'m-memo-email\')">Fechar</button>';
  ...
  return {
    copiarEmail: copiarEmail, preverEmail: preverEmail,
