...
var Memorandos = (function() {
  ...
  function classificar(m) {
    ...
    // 2. Os Meus — criador vê sempre os seus (até concluir)
    if (euCriei) return 2;

    // 3. Restantes fluxos — DDS vê tudo
    if (APP.isDds()) {
      ...
      // memos criados por DDS sem criado_por preenchido (dados antigos)
      if (m.criado_por_dds)      return 3;  // A Enviar por defeito
    }
  ...
  function getModaisHTML() {
    ...
    html += '<div class="fi"><label class="fl">Equipamento</label><select id="m-memo-eq" class="fin"><option value="">Sem equipamento associado</option></select></div>';
