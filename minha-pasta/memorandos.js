...
var Memorandos = (function() {
  ...
  function getBadges() {
    // sec1: recebidos de equipamentos (não DDS, não aprovados)
    var s1 = APP.memos.filter(function(m) {
      return m.estado !== 'concluido' && !m.criado_por_dds && !m.aprovado_dds;
    }).length;
    // sec2: criados pelo utilizador actual
    var s2 = APP.memos.filter(function(m) {
      return m.estado !== 'concluido' && APP.user && m.criado_por === APP.user.id;
    }).length;
    // sec3: aprovados, por enviar
    var s3 = APP.memos.filter(function(m) {
      return m.estado !== 'concluido' && m.aprovado_dds && !m.data_envio_servicos;
    }).length;
    // sec4: enviados, em curso
    var s4 = APP.memos.filter(function(m) {
      return m.estado !== 'concluido' && m.data_envio_servicos;
    }).length;
  ...
  function criar() {
    ...
    var obs = document.getElementById('m-memo-obs');
    // criado_por_dds é automaticamente true se o utilizador for DDS
    var cDds = APP.isDds();
  ...
  function pedirConf(id) {
    ...
    if (!m) return;
    // destinatario_id só se for UUID válido
    var destId = null;
    if (m.criado_por) {
      var uuidRx = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
      if (uuidRx.test(m.criado_por)) destId = m.criado_por;
    }
    sbPost('avisos', {
