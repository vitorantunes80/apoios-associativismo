...
var Memorandos = (function() {
  ...
  function classificar(m) {
    ...
    // 2. Os Meus — criador vê SEMPRE os seus memorandos aqui (até concluir)
    //    É a vista de acompanhamento pessoal
    if (euCriei) return 2;

    // Para os restantes (não criados pelo utilizador actual):
    // 3. DDS vê os fluxos
    if (APP.isDds()) {
      if (m.data_envio_servicos) return 4;    // Em Curso
      if (m.aprovado_dds)        return 3;    // A Enviar
      if (!m.criado_por_dds)     return 1;    // Recebidos de equipamento
    }
  ...
  function getBadges() {
    // sec2 — Os Meus: todos os do utilizador actual não concluídos
    var s2 = APP.memos.filter(function(m) {
      return m.estado !== 'concluido' && APP.user && m.criado_por === APP.user.id;
    }).length;

    // sec1, sec3, sec4 — contam memorandos NÃO criados pelo utilizador actual
    // (para não duplicar com sec2)
    var s1=0, s3=0, s4=0;
    APP.memos.forEach(function(m) {
      if (m.estado === 'concluido') return;
      var euCriei = APP.user && m.criado_por === APP.user.id;
      if (euCriei) return; // já contado em s2
      if (m.data_envio_servicos) { s4++; return; }
      if (m.aprovado_dds) { s3++; return; }
      if (!m.criado_por_dds) s1++;
    });
  ...
  function getModaisHTML() {
    ...
    html += '<div class="fi"><label class="fl">Equipamento *</label><select id="m-memo-eq" class="fin"><option value="">Seleccione o equipamento...</option></select></div>';
  ...
  function openNovo() {
    ...
    // equipamentos — admin/resp_dds vê todos; outros só o seu
    var eqSel = document.getElementById('m-memo-eq');
    if (eqSel) {
      eqSel.innerHTML = '<option value="">Seleccione o equipamento...</option>';
      var eqList = APP.equipamentos;
      // não-DDS só vê o seu equipamento
      if (!APP.isDds() && APP.user && APP.user.equipamento_id) {
        eqList = eqList.filter(function(e) { return e.id === APP.user.equipamento_id; });
      }
  ...
  function criar() {
    ...
    var eqSelVal = document.getElementById('m-memo-eq');
    if (!eqSelVal || !eqSelVal.value) { toast('Seleccione o equipamento.', 'error'); return; }
    ...
    gerarNumMemo().then(function(num) {
      ...
      return sbPost('memorandos_dds', {
        ...
        aprovado_dds:   cDds,
        num_reforcos:   0,
        equipamento_id: (document.getElementById('m-memo-eq') && document.getElementById('m-memo-eq').value) ? document.getElementById('m-memo-eq').value : (APP.spotEqId || null)
      });
    }).then(function() {
