...
var Equipamentos = (function() {
  ...
  function getModaisHTML() {
    ...
    html += '<div class="form-sec-t">Espaços / Valências para reservas</div>';
    html += '<div class="form-hint" style="margin-bottom:8px;">Defina os espaços que podem ser reservados e o que contar em cada um (ex: Parque de Campismo → Nº de pessoas + Nº de tendas).</div>';
    html += '<div id="eq-espacos-list"></div>';
    html += '<button class="btn btn-ghost btn-sm" onclick="Equipamentos.addEspaco()" style="margin-top:6px;">+ Adicionar espaço</button>';
    html += '</div>';
  ...
  function openEditar(id) {
    ...
    // carregar espaços deste equipamento
    var espacos = (APP.espacos || []).filter(function(x){ return x.equipamento_id === id; });
  ...
  function salvar() {
    ...
    p.then(function(data) {
      var eqId = APP.editId || (Array.isArray(data) ? data[0].id : (data.id || null));
      _salvarEspacos(eqId, function() {
