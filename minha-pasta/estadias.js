...
var Estadias = (function() {
  ...
  function renderPainel(eqId) {
    var eq = getEq(eqId);
    var pode = APP.podeGerirEquipamento ? APP.podeGerirEquipamento(eqId) : true;
  ...
  function _renderLista(lista, pode, historico) {
    ...
    lista.forEach(function(e) {
      ...
      h += '<td class="td-m">' + _fdata(e.data_checkin) + '</td>';
      h += '<td class="td-m">';
      if (e.estado === 'ativa') {
  ...
  function _renderStats() {
    var fechadas = _lista.filter(function(e) { return e.estado !== 'cancelada'; });
    if (!fechadas.length) return emptyState('Sem dados', 'Ainda não há estadias para analisar.');
  ...
  function salvar() {
    ...
    p.then(function() {
      ...
      load(_eqId, function() { App.renderContent(); });
    }).catch(function(e) {
  ...
  function imprimir() {
    ...
    var h = '<div class="pr-page">';
    h += '<div class="pr-head"><div class="pr-org">Município de Cabeceiras de Basto · Divisão de Desenvolvimento Social</div>';
    h += '<div class="pr-title">Estatísticas de Ocupação</div>';
    h += '<div class="pr-meta">' + (eq ? eq.nome : '') + ' · Ano ' + ano + ' · Emitido em ' + _fdata(new Date().toISOString()) + '</div></div>';
    ...
    h += '<div class="pr-kpi"><div class="pr-kpi-v">' + (fechadas.length ? (totNoites / fechadas.length).toFixed(1) : 0) + '</div><div class="pr-kpi-l">Estadia média</div></div>';
    h += '</div>';
    ...
    h += '<div class="pr-foot"><div>GestDDS · Divisão de Desenvolvimento Social · Município de Cabeceiras de Basto</div></div>';
    h += '</div>';
  ...
  return {
    ...
    setTab: setTab, imprimir: imprimir,
