...
var Dashboard = (function() {
  ...
  function _renderSpotlight() {
    ...
    var temEstadias = spotEq && typeof modAtivo === 'function' && modAtivo(spotEq, 'estadias')
                   && typeof Estadias !== 'undefined'
                   && (typeof APP.podeVerEstadias !== 'function' || APP.podeVerEstadias(spotEq.id));
    if (temEstadias) tabs.push('Check-in / Check-out');
    var temVisitas = spotEq && typeof modAtivo === 'function' && modAtivo(spotEq, 'visitas')
                  && typeof Visitas !== 'undefined'
                  && (typeof APP.podeVerVisitas !== 'function' || APP.podeVerVisitas(spotEq.id));
    var idxVisitas = -1;
    if (temVisitas) { idxVisitas = tabs.length; tabs.push('Registo de Visitas'); }
    ...
    // Tab 6 — Check-in / Check-out (só se o módulo estiver activo)
    if (temEstadias) {
      html += '<div class="eq-tab-panel" id="spot-t6">';
      html += '<div id="spot-estadias-content"><div class="loading-wrap"><div class="spinner"></div></div></div>';
      html += '</div>';
    }
    // Tab do Registo de Visitas
    if (temVisitas) {
      APP._idxVisitas = idxVisitas;
      html += '<div class="eq-tab-panel" id="spot-t' + idxVisitas + '">';
  ...
  function _renderChecklistsTab(eqId) {
    ...
    html += '<button class="btn btn-primary btn-sm" onclick="App.nav(\'checklists\')">Abrir checklists</button></div>';
    ...
    hours.forEach(function(h, hi) {
      ...
      dias.forEach(function(d) {
        ...
        escalas.forEach(function(e) {
          ...
          // começa nesta hora?
          var horaIni = e.hora_entrada ? parseInt(e.hora_entrada.split(':')[0],10) : 7;
          if (horaIni !== hourNum) return;

          var horaFim = e.hora_saida ? parseInt(e.hora_saida.split(':')[0],10) : horaIni + 8;
          ...
          var dur = Math.max(1, horaFim - horaIni);
          var height = dur * 26 - 3;
          var sub = e.funcao || TURNO_LABELS[e.tipo_turno] || '';

          html += '<div class="sched-event" style="background:' + cc.bg + ';color:' + cc.color + ';height:' + height + 'px;cursor:pointer;" onclick="App.nav(\'escalas\')">';
          html += H(nome) + '<br><span style="font-weight:400;opacity:.75;font-size:9.5px;">' + H(sub) + '</span>';
          html += '</div>';
        });
        html += '</div>';
      });
  ...
  function _renderChecklists() {
    // registos recentes de checklists dos equipamentos visíveis
    var eqIds = APP.isDds() ? APP.equipamentos.map(function(e){return e.id;}) : (APP.user && APP.user.equipamento_id ? [APP.user.equipamento_id] : []);
    ...
    var html = '<div class="panel-card">';
    html += '<div class="panel-head"><div class="panel-title">Checklists Pendentes</div><div class="panel-link" onclick="App.nav(\'checklists\')">Ver todas</div></div>';
    if (!pendentes.length) {
      html += '<div style="padding:14px 13px;font-size:12.5px;color:var(--text-3);text-align:center;">Sem checklists pendentes. ✓</div>';
    } else {
  ...
  function _renderReparacoes() {
    // memorandos abertos dos equipamentos visíveis
    var memos = APP.memos.filter(function(m){ return m.estado !== 'concluido'; });
    if (!APP.isDds() && APP.user && APP.user.equipamento_id) {
      memos = memos.filter(function(m){ return m.equipamento_id === APP.user.equipamento_id; });
    }
    memos.sort(function(a,b){ return new Date(b.created_at)-new Date(a.created_at); });

    var html = '<div class="panel-card">';
    html += '<div class="panel-head"><div class="panel-title">Reparações em Curso</div><div class="panel-link" onclick="App.nav(\'memorandos\',\'sec2\')">Ver todas</div></div>';
    if (!memos.length) {
      html += '<div style="padding:14px 13px;font-size:12.5px;color:var(--text-3);text-align:center;">Sem reparações em curso.</div>';
    } else {
  ...
  function switchTab(i) {
    ...
    // panels (0..7 — 6 = check-in/check-out, 7 = visitas, quando existem)
    for (var j = 0; j <= 7; j++) {
      var p = document.getElementById('spot-t' + j);
      if (p) p.classList.toggle('active', j === i);
    }
    // carregar o conteúdo das estadias na primeira abertura
    if (i === 6 && document.getElementById('spot-estadias-content') && typeof Estadias !== 'undefined') {
      var eqId = APP.spotEqId;
    ...
    // registo de visitas
    if (typeof Visitas !== 'undefined' && document.getElementById('spot-visitas-content')
        && i === APP._idxVisitas) {
      var eqV = APP.spotEqId;
      Visitas.load(eqV, function() {
        var c = document.getElementById('spot-visitas-content');
        if (c) c.innerHTML = Visitas.renderPainel(eqV);
      });
    }
  }
