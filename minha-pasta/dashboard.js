...
var Dashboard = (function() {
  ...
  function render() {
    ...
    // contagem restrita aos equipamentos que o utilizador pode ver
    var mAbertos = APP.memos.filter(function(m) {
      if (m.estado === 'concluido') return false;
      if (!m.equipamento_id) return APP.isDds();
    ...
    if (APP.isDds()) {
      ...
      } else if (APP.acessoMenu('checklists') && APP.moduloDoMeuEq('checklist')) {
      ...
      if (t.k === 'geral') {
        if (spotEq) {
          ...
          if (_tem('reservas')) {
            ...
            // No campismo, o que interessa é a ocupação. Vai SEMPRE ao servidor:
            // ler Estadias.lista() dava valores obsoletos (ou zero) porque essa
            // lista só é carregada ao abrir o separador Check-in/Check-out.
            html += _statCard('si-blue', _svgCal(), 'Ocupação',
                              '<span id="spot-ocup-v">—</span>',
                              '', '');
            (function(eqOcup) {
              Estadias.ocupacao(eqOcup, function(err, o) {
                var elV = document.getElementById('spot-ocup-v');
                if (!elV) return; // o utilizador já mudou de ecrã
                var elS = elV.parentNode.parentNode.querySelector('.stat-sub');
                if (err) {
                  elV.textContent = '—';
                  if (elS) elS.textContent = 'erro ao actualizar';
                  return;
                }
                elV.textContent = o.lugares;
                if (elS) elS.textContent = o.pessoas + (o.pessoas === 1 ? ' pessoa no parque' : ' pessoas no parque');
              });
            })(spotEq.id);
          }
  ...
  function _renderEscala() {
    ...
    // ── Mini-tabela funcionário × dia (mesmo formato da página de Escalas) ──
    // A grelha horária empilhava vários funcionários na mesma coluna e ficava
    // ilegível. Uma tabela por funcionário lida bem com turnos partidos.
    var funcs = (APP.utilizadores || []).filter(function(u) {
      return funcNoEq(u, eqId) && !u.isDds;
    });

    html += '<div class="dash-esc-wrap"><table class="dash-esc">';
    html += '<thead><tr><th class="dash-esc-fn">Funcionário</th>';
    dias.forEach(function(d) {
      var isHoje = d.toDateString() === hoje.toDateString();
      html += '<th' + (isHoje ? ' class="today"' : '') + '>' + nomesDia[d.getDay()] + '<br>' + d.getDate() + '</th>';
    });
    html += '</tr></thead><tbody>';

    funcs.forEach(function(f) {
      html += '<tr><td class="dash-esc-fn">' + H(_primeiroNome(f.nome)) + '</td>';
      dias.forEach(function(d) {
        var dataStr = d.getFullYear() + '-' + ('0'+(d.getMonth()+1)).slice(-2) + '-' + ('0'+d.getDate()).slice(-2);
        // todos os turnos deste funcionário neste dia (pode haver 2: manhã+tarde)
        var turnos = escalas.filter(function(e) {
          if (e.funcionario_id !== f.id) return false;
          var ei = String(e.data_inicio).slice(0, 10);
          var ef = String(e.data_fim || e.data_inicio).slice(0, 10);
          return dataStr >= ei && dataStr <= ef;
        }).sort(function(a, b) {
          return (_hDec(a.hora_entrada) || 0) - (_hDec(b.hora_entrada) || 0);
        });

        html += '<td>';
        if (turnos.length) {
          turnos.forEach(function(t) {
            var cc = TURNO_CORES[t.tipo_turno] || TURNO_CORES.normal;
            var txt = (t.hora_entrada && t.hora_saida)
              ? t.hora_entrada.slice(0,5) + '–' + t.hora_saida.slice(0,5)
              : (TURNO_LABELS[t.tipo_turno] || '');
            html += '<div class="dash-esc-t" style="background:' + cc.bg + ';color:' + cc.color + ';">' + H(txt) + '</div>';
          });
        } else {
          html += '<span class="dash-esc-v">·</span>';
        }
        html += '</td>';
      });
      html += '</tr>';
  ...
  function _renderMemosRecentes() {
    // Só memorandos de equipamentos que o utilizador pode ver: um funcionário
    // do CEAVM não tem que saber das reparações do Centro Hípico.
    var abertos = APP.memos.filter(function(m) {
      if (m.estado === 'concluido') return false;
      if (!m.equipamento_id) return APP.isDds(); // memos transversais: só DDS
      return APP.podeVerEquipamento(m.equipamento_id);
    }).slice(0, 4);

    var html = '<div class="sec-head" style="margin-top:14px;"><div class="sec-title">Memorandos Abertos</div>';
    if (APP.podeCriarMemorandos()) {
      html += '<div class="sec-link" onclick="App.nav(\'memorandos\',\'sec2\')">Ver todos</div>';
    }
    html += '</div>';
