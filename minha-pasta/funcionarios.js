...
var Funcionarios = (function() {
  ...
  function darAcesso(id) {
    // DEBUG: PATCH direto + releitura, com alertas
    fetch(SB_URL + '/rest/v1/utilizadores?id=eq.' + id, {
      method: 'PATCH',
      mode: 'cors', credentials: 'omit',
      headers: sbHeaders(),
      body: JSON.stringify({ tem_acesso: true })
    }).then(function(r) {
      return r.text().then(function(t) {
        alert('PATCH → status ' + r.status + '\nresposta: ' + (t || '(vazio)'));
        // reler para confirmar
        return fetch(SB_URL + '/rest/v1/utilizadores?id=eq.' + id + '&select=nome,tem_acesso', {
          mode: 'cors', credentials: 'omit', headers: sbHeaders()
        }).then(function(r2) { return r2.text(); }).then(function(t2) {
          alert('RELEITURA da BD → ' + t2);
