...
var Funcionarios = (function() {
  ...
  function render() {
    ...
    APP.equipamentos.forEach(function(eq) {
      ...
      html += _tabela(groups[eq.id], isAdmin);
      html += '</div>';
    });
  ...
  function _tabela(funcs, isAdmin) {
    ...
    html += '<thead><tr><th>Nome</th><th>Perfil</th><th>Código</th><th>Contacto</th><th>Estado</th>';
    ...
    funcs.forEach(function(u) {
      ...
      if (u.username) html += '<div class="td-m">' + H(u.username) + '</div>';
      html += '</div></div></td>';
      ...
      html += '<td class="td-m">' + H(u.email || '—') + '</td>';
      html += '<td>' + (ativo
  ...
  function openNovo() {
    ...
    ['func-nome','func-codigo','func-email','func-username'].forEach(function(id) {
