...
var APP = {
  ...
  // RELEVÂNCIA — o módulo está activo no equipamento do utilizador?
  // Serve para esconder menus que não fazem sentido (ex.: Reservas no
  // Parque de Campismo). Utilizadores DDS vêem tudo (transversal).
  moduloDoMeuEq: function(k) {
    if (!APP.user) return false;
    if (APP.user.isDds || APP.user.perfil === 'admin') return true;
    var eqId = APP.user.equipamento_id;
    if (!eqId) return true; // sem equipamento afecto, não filtrar
    var eq = (APP.equipamentos || []).filter(function(e) { return e.id === eqId; })[0];
    if (!eq) return true;
    return (typeof modAtivo === 'function') ? modAtivo(eq, k) : true;
  },
