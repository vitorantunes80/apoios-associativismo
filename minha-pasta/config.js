...
var APP = {
  ...
  _menuPorPerfil: function(menu) {
    ...
    switch (menu) {
      ...
      // check-in/check-out: dados pessoais de hóspedes — acesso restrito.
      // O admin autoriza cada funcionário individualmente (permissoes_extra).
      case 'estadias':     return !!APP.user.isDds || APP.isResponsavel();
      case 'visitas':      return !!APP.user.isDds || APP.isResponsavel();
  ...
  // ── EQUIPAMENTO × MÓDULO ─────────────────────────────────
  // Reservas, Escalas e Checklists são SEMPRE de um equipamento.
  // Estes três helpers são a única fonte de verdade sobre "que
  // equipamento devo mostrar neste módulo".

  // Equipamentos que o utilizador vê E onde o módulo está ligado
  eqsComModulo: function(k) {
    if (!APP.user) return [];
    return (APP.equipamentos || []).filter(function(e) {
      if (!APP.podeVerEquipamento(e.id)) return false;
      return (typeof modAtivo === 'function') ? modAtivo(e, k) : true;
    });
  },

  // O módulo faz sentido para este utilizador? (esconder menus vazios)
  moduloDoMeuEq: function(k) {
    return APP.eqsComModulo(k).length > 0;
  },

  // Qual o equipamento a mostrar no módulo k.
  // Preferência: o que está seleccionado — mas só se lá tiver o módulo.
  // Caso contrário, o primeiro equipamento válido. null = nenhum.
  eqParaModulo: function(k) {
    var validos = APP.eqsComModulo(k);
    if (!validos.length) return null;
    var atual = validos.filter(function(e) { return e.id === APP.spotEqId; })[0];
    return atual ? atual.id : validos[0].id;
  },
  ...
  // pode ver o check-in/check-out deste equipamento?
  // (funcionário só se o admin lhe atribuir acesso em permissoes_extra)
  podeVerEstadias: function(eqId) {
    if (!APP.user) return false;
    if (!APP.acessoMenu('estadias')) return false;
    if (!APP.podeVerEquipamento(eqId)) return false;
    return true;
  },

  // pode registar check-in / check-out?
  podeGerirEstadias: function(eqId) {
    if (!APP.podeVerEstadias(eqId)) return false;
    if (APP._veTudo()) return true;
    if (APP._unidadeChefe()) return APP._eqNaMinhaUnidade(eqId);
    if (APP._ehPerfilResponsavel()) return APP._ehResponsavelDe(eqId);
  ...
  // registo de visitas — mesma lógica das estadias
  podeVerVisitas: function(eqId) {
    if (!APP.user) return false;
    if (!APP.acessoMenu('visitas')) return false;
    return APP.podeVerEquipamento(eqId);
  },
  podeGerirVisitas: function(eqId) {
    if (!APP.podeVerVisitas(eqId)) return false;
    if (APP._veTudo()) return true;
    if (APP._unidadeChefe()) return APP._eqNaMinhaUnidade(eqId);
    if (APP._ehPerfilResponsavel()) return APP._ehResponsavelDe(eqId);
    return funcNoEq(APP.user, eqId);
...
var MOD_DEFS = [
  ...
  { k: 'estadias',     l: 'Check-in / Check-out',  s: 'Estadias, ocupação e dormidas',     c: 'si-green' },
  { k: 'visitas',      l: 'Registo de Visitas',    s: 'Visitantes, escolas e grupos',      c: 'si-purple' },
...
// Um módulo (Reservas/Escalas/Checklists) mostra sempre UM equipamento.
// Estas três funções garantem que nunca se aterra num equipamento onde
// o módulo está desligado.

// Que equipamento mostrar. Honra o pedido se for válido; senão resolve.
function eqDoModulo(k, eqIdPedido) {
  if (eqIdPedido) {
    var ok = APP.eqsComModulo(k).filter(function(e) { return e.id === eqIdPedido; }).length > 0;
    if (ok) return eqIdPedido;
  }
  return APP.eqParaModulo(k);
}

// Selector no cabeçalho do módulo — só lista equipamentos onde ele está ligado.
function eqSwitcher(k, eqId) {
  var eqs = APP.eqsComModulo(k);
  if (eqs.length <= 1) return ''; // com um só equipamento, o selector é ruído
  var s = '<select class="eq-switch" onchange="trocarEqModulo(this.value)">';
  eqs.forEach(function(e) {
    s += '<option value="' + e.id + '"' + (e.id === eqId ? ' selected' : '') + '>' + H(e.nome) + '</option>';
  });
  s += '</select>';
  return s;
}

// Trocar de equipamento dentro de um módulo: actualiza a selecção global,
// por isso a sidebar e o dashboard ficam coerentes com o que se está a ver.
function trocarEqModulo(eqId) {
  APP.spotEqId = eqId;
  try { App.buildSidebar(); } catch (e) {}
  // Alguns módulos precisam de ir buscar dados do novo equipamento antes de
  // desenhar (senão a grelha aparece vazia). Recarregar consoante a vista.
  var v = APP.view;
  if (v === 'escalas'   && typeof Escalas   !== 'undefined' && Escalas.load) {
    Escalas.load(function() { App.renderContent(); }); return;
  }
  if (v === 'reservas'  && typeof Reservas  !== 'undefined' && Reservas.load) {
    Reservas.load(function() { App.renderContent(); }); return;
  }
  if (v === 'checklists' && typeof Checklist !== 'undefined' && Checklist.load) {
    Checklist.load(function() { App.renderContent(); }); return;
  }
  App.renderContent();
}

// Estado vazio quando NENHUM equipamento acessível tem o módulo ligado.
function semEqParaModulo(nomeModulo) {
  return '<div class="mod-ph"><div class="mod-ph-t">Sem equipamentos com ' + nomeModulo + '</div>' +
         '<div class="mod-ph-s">Este módulo não está activo em nenhum equipamento a que tenha acesso. ' +
         'Pode ligá-lo na ficha do equipamento, em Módulos.</div></div>';
}
...
// Reservas recorrentes (permanentes) estão permitidas neste equipamento?
// Guardado em eq.modulos.reservas_recorrentes — não precisa de coluna nova.
// Ausente => permitido (mantém o comportamento anterior).
function permiteRecorrente(eq) {
  if (!eq) return false;
  var ov = eq.modulos && typeof eq.modulos === 'object' ? eq.modulos.reservas_recorrentes : undefined;
  return ov !== false; // só desligado quando explicitamente false
}
