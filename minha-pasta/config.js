...
var APP = {
  ...
  _menuPorPerfil: function(menu) {
    ...
    switch (menu) {
      ...
      default:             return true;  // reservas, escalas, checklists, ferias, horas, dashboard
    }
  },
  ...
  // (responsabilidade na ficha do equipamento — pode ter vários;
  //  mantém-se compatível com o antigo equipamento_id no utilizador)
  ...
  podeVerEquipamento: function(eqId) {
    ...
    if (APP._unidadeChefe()) return APP._eqNaMinhaUnidade(eqId);
    if (APP._ehPerfilResponsavel()) return APP._ehResponsavelDe(eqId);
    return APP.user.equipamento_id === eqId; // funcionário: só o afeto
  },
  ...
  // pode preencher checklists? (funcionário é só consulta)
