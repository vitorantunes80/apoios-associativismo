...
var APP = {
  ...
  podeGerirEquipamento: function(eqId) {
    ...
    if (APP.user.perfil === 'admin' || APP.user.isDds) return true;
    // responsável só gere o seu
    if (['responsavel','professor_responsavel'].indexOf(APP.user.perfil) >= 0) {
      return APP.user.equipamento_id === eqId;
    }
    return false;
  },
  ...
  // pode criar memorandos?
  podeCriarMemorandos: function() {
    if (!APP.user) return false;
    // todos excepto funcionário simples
    return APP.user.perfil !== 'funcionario';
  },
  ...
  // pode preencher checklists? (todos podem)
  podePreencherChecklist: function(eqId) {
    if (!APP.user) return false;
    return APP.podeVerEquipamento(eqId);
  },

  // pode gerir templates de checklist? (admin, DDS, responsável)
...
function loadAll(cb) {
  ...
  Promise.all([
    ...
    sbGet('reservas', 'order=data_inicio.asc').then(function(d) {
      APP.reservas = Array.isArray(d) ? d : [];
    }).catch(function() { APP.reservas = []; }),

    sbGet('escalas', 'order=data_inicio.asc').then(function(d) {
