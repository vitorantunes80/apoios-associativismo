...
var Reservas = (function() {
  ...
  // Desvio de fuso local no formato '+HH:MM' para uma data/hora dada, para
  // gravar timestamps que não "andam" quando a coluna é timestamptz.
  function _offsetLocal(isoSemTz) {
    var off = -new Date(isoSemTz).getTimezoneOffset(); // minutos a leste de UTC
    var sinal = off >= 0 ? '+' : '-';
    off = Math.abs(off);
    return sinal + _pad(Math.floor(off / 60)) + ':' + _pad(off % 60);
  }
  ...
  function _expandirRecorrentes(reservas, dias) {
    ...
    reservas.forEach(function(r) {
      if (!r.recorrente) {
        // DIA vem de data_inicio (parte YYYY-MM-DD). HORA vem de hora_inicio/fim
        // (texto puro), que não sofre a conversão UTC do timestamptz. Ler a hora
        // de data_inicio dava a hora UTC e deslocava o bloco no calendário.
        var si = String(r.data_inicio);
        var sf = String(r.data_fim || r.data_inicio);
        var diaI = si.slice(0, 10), diaF = sf.slice(0, 10);
        var hIni = _hDec(r.hora_inicio);
        var hFim = _hDec(r.hora_fim);
        if (hIni == null) hIni = si.length >= 16 ? _hDec(si.slice(11, 16)) : 9;
        if (hFim == null) hFim = sf.length >= 16 ? _hDec(sf.slice(11, 16)) : (hIni + 1);
        if (hFim == null) hFim = hIni + 1;
        // se acaba noutro dia, mostra até ao fim do dia
        if (diaF !== diaI) hFim = 24;

        dias.forEach(function(d) {
          var dStr = d.getFullYear() + '-' + _pad(d.getMonth() + 1) + '-' + _pad(d.getDate());
          if (dStr >= diaI && dStr <= diaF) {
  ...
  // ── LISTAGEM ──────────────────────────────────────────────
  // mês em foco na lista (YYYY-MM) — null = todas
  var _listaEqId = null;
  function openListagem(eqId) {
    _listaEqId = eqId;
    // arrancar no mês da semana vista; se esse mês não tiver reservas,
    // saltar para o primeiro mês que tenha (evita abrir numa lista vazia).
    var base = _getWeekDates ? _getWeekDates()[0] : new Date();
    var ymBase = base.getFullYear() + '-' + _pad(base.getMonth() + 1);
    var comReservas = {};
    (APP.reservas || []).forEach(function(r) {
      if (r.equipamento_id === eqId) {
        var ym = String(r.data_inicio).slice(0, 7);
        if (ym.length === 7) comReservas[ym] = true;
      }
    });
    if (!comReservas[ymBase]) {
      var keys = Object.keys(comReservas).sort();
      // primeiro mês >= mês actual, senão o mais recente
      var futuro = keys.filter(function(k) { return k >= ymBase; });
      ymBase = futuro.length ? futuro[0] : (keys.length ? keys[keys.length - 1] : ymBase);
    }
    APP.resListaYM = ymBase;
    _desenharListagem();
    openModal('m-res-listagem');
  }

  function listaMes(ym) { APP.resListaYM = ym; _desenharListagem(); }

  var _MESES_L = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];

  function _desenharListagem() {
    var eqId = _listaEqId;
    var body = document.getElementById('m-res-list-body');
    if (!body) return;

    var todas = (APP.reservas || []).filter(function(r) { return r.equipamento_id === eqId; });

    // que meses têm reservas (para os botões)
    var mesesComReservas = {};
    todas.forEach(function(r) {
      var ym = String(r.data_inicio).slice(0, 7);
      if (ym.length === 7) mesesComReservas[ym] = true;
    });
    var ymsOrdenados = Object.keys(mesesComReservas).sort();

    var ymSel = APP.resListaYM || null;
    // filtrar pelo mês (ou todas)
    var reservas = (!ymSel || ymSel === 'todas')
      ? todas
      : todas.filter(function(r) { return String(r.data_inicio).slice(0, 7) === ymSel; });
    reservas.sort(function(a, b) { return String(a.data_inicio).localeCompare(String(b.data_inicio)); });

    // barra de meses
    var html = '<div class="rep-periodo" style="margin-bottom:12px;">';
    html += '<button class="rep-mes' + (ymSel === 'todas' ? ' active' : '') + '" onclick="Reservas.listaMes(\'todas\')">Todas</button>';
    ymsOrdenados.forEach(function(ym) {
      var m = parseInt(ym.slice(5, 7), 10);
      var lbl = _MESES_L[m - 1].slice(0, 3) + ' ' + ym.slice(0, 4);
      html += '<button class="rep-mes' + (ym === ymSel ? ' active' : '') + '" onclick="Reservas.listaMes(\'' + ym + '\')">' + lbl + '</button>';
    });
    html += '</div>';

    // rótulo + contagem
    var rot = (!ymSel || ymSel === 'todas') ? 'Todas as reservas'
            : _MESES_L[parseInt(ymSel.slice(5, 7), 10) - 1] + ' de ' + ymSel.slice(0, 4);
    html += '<div class="est-hist-h">' + reservas.length + (reservas.length === 1 ? ' reserva' : ' reservas') + ' · ' + H(rot) + '</div>';

    if (!reservas.length) {
      body.innerHTML = html + emptyState('Sem reservas', 'Não há reservas em ' + rot + '.');
      return;
    }
    ...
    reservas.forEach(function(r) {
      ...
      if (r.recorrente) {
        ...
        var _si = String(r.data_inicio), _sf = String(r.data_fim || r.data_inicio);
        var _hi = r.hora_inicio ? String(r.hora_inicio).slice(0, 5) : (_si.length >= 16 ? _si.slice(11, 16) : '');
        var _hf = r.hora_fim ? String(r.hora_fim).slice(0, 5) : (_sf.length >= 16 ? _sf.slice(11, 16) : '');
        html += '<td class="td-mono">' + _hi + '–' + _hf + '</td>';
  ...
  function getModaisHTML() {
    ...
    html += '<div class="form-row"><div class="fi"><label class="fl">Contacto (telemóvel)</label><input type="tel" id="res-contacto" class="fin" inputmode="tel" placeholder="Ex: 912 345 678"></div></div>';
  ...
  function abrirDetalhe(id) {
    ...
    if (r.recorrente) {
      ...
      var sdi = String(r.data_inicio), sdf = String(r.data_fim || r.data_inicio);
      var hhi = r.hora_inicio ? String(r.hora_inicio).slice(0, 5) : (sdi.length >= 16 ? sdi.slice(11, 16) : '');
      var hhf = r.hora_fim ? String(r.hora_fim).slice(0, 5) : (sdf.length >= 16 ? sdf.slice(11, 16) : '');
      quando = fData(r.data_inicio) + ' · ' + hhi + '–' + hhf;
    }
    ...
    body += linha('Contacto', r.contacto);
  ...
  function _resetModal() {
    ...
    var ct = document.getElementById('res-contacto'); if (ct) ct.value = '';
  ...
  function openEditar(idOrEl) {
    ...
    if (r.recorrente) {
      ...
      document.getElementById('res-tipo-p').checked = true;
      // A data vem da string data_inicio (só a parte YYYY-MM-DD, imune a fuso).
      // A HORA vem de hora_inicio/hora_fim, que são TEXTO puro e não sofrem a
      // conversão UTC do timestamptz. Ler a hora de data_inicio dava a hora UTC
      // (ex.: 09:00 local guardado como 08:00Z aparecia como 08:00).
      if (r.data_inicio) {
        document.getElementById('res-data-i').value = String(r.data_inicio).slice(0, 10);
      }
      var hIni = r.hora_inicio ? String(r.hora_inicio).slice(0, 5)
               : (String(r.data_inicio).length >= 16 ? String(r.data_inicio).slice(11, 16) : '09:00');
      var hFim = r.hora_fim ? String(r.hora_fim).slice(0, 5)
               : (String(r.data_fim).length >= 16 ? String(r.data_fim).slice(11, 16) : '10:00');
      document.getElementById('res-inicio').value = hIni;
      document.getElementById('res-fim').value    = hFim;
    }
    onTipoChange();
  ...
  function salvar() {
    ...
    var body = {
      ...
      contacto:       (function(){ var c=document.getElementById('res-contacto'); return c && c.value.trim() ? c.value.trim() : null; })(),
    ...
    if (isRec) {
      ...
      // Reserva pontual = SEMPRE um único dia. Gravar com o fuso local explícito
      // (ex.: +01:00) para que, se a coluna for timestamptz, a hora não "ande"
      // ao voltar do servidor. hora_inicio/fim ficam também como texto simples.
      var tz = _offsetLocal(dataI + 'T' + horaI + ':00');
      body.data_inicio = dataI + 'T' + horaI + ':00' + tz;
      body.data_fim    = dataI + 'T' + horaF + ':00' + tz;
  ...
  return {
    ...
    openListagem: openListagem, listaMes: listaMes, confirmar: confirmar, salvar: salvar, apagar: apagar,
