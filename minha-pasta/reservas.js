...
var Reservas = (function() {
  ...
  function _expandirRecorrentes(reservas, dias) {
    ...
    reservas.forEach(function(r) {
      if (!r.recorrente) {
        // Extrair dia e hora DA STRING (sem new Date), para o fuso não deslocar.
        var si = String(r.data_inicio);
        var sf = String(r.data_fim || r.data_inicio);
        var diaI = si.slice(0, 10), diaF = sf.slice(0, 10);
        var hIni = si.length >= 16 ? _hDec(si.slice(11, 16)) : (_hDec(r.hora_inicio) || 9);
        var hFim = sf.length >= 16 ? _hDec(sf.slice(11, 16)) : (_hDec(r.hora_fim) || (hIni + 1));
    ...
    if (!reservas.length) {
      ...
      reservas.forEach(function(r) {
        ...
        if (r.recorrente) {
          ...
          var _hi = _si.length >= 16 ? _si.slice(11, 16) : String(r.hora_inicio || '').slice(0, 5);
          var _hf = _sf.length >= 16 ? _sf.slice(11, 16) : String(r.hora_fim || '').slice(0, 5);
  ...
  function abrirDetalhe(id) {
    ...
    if (r.recorrente) {
      ...
      var hhi = sdi.length >= 16 ? sdi.slice(11, 16) : String(r.hora_inicio || '').slice(0, 5);
      var hhf = sdf.length >= 16 ? sdf.slice(11, 16) : String(r.hora_fim || '').slice(0, 5);
  ...
  function openEditar(idOrEl) {
    ...
    if (r.recorrente) {
      ...
      document.getElementById('res-tipo-p').checked = true;
      // Ler data e hora DIRECTAMENTE da string 'YYYY-MM-DDTHH:MM...'. Passar por
      // new Date() convertia timestamptz do servidor para hora local (ex.: 11:00
      // guardado como +00 aparecia 12:00), e ao guardar a hora "andava".
      if (r.data_inicio) {
        var si = String(r.data_inicio);
        document.getElementById('res-data-i').value = si.slice(0, 10);
        var hi = si.length >= 16 ? si.slice(11, 16) : (r.hora_inicio ? String(r.hora_inicio).slice(0, 5) : '09:00');
        document.getElementById('res-inicio').value = hi;
      }
      if (r.data_fim && String(r.data_fim).length >= 16) {
        document.getElementById('res-fim').value = String(r.data_fim).slice(11, 16);
      } else if (r.hora_fim) {
