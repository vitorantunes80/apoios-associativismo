...
;          html += '<div style="font-weight:700;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">';
  ...
  function _configurarPorTipo(eqId) {
    var eq = getEq(eqId);
    var tipo = eq ? getTipo(eq.tipo_id) : null;

    // popular dropdown de espaços
    var sel = document.getElementById('res-espaco-sel');
    if (sel) {
      sel.innerHTML = '<option value="">Nenhum</option>';
      if (tipo && tipo.espacos) {
        tipo.espacos.split(',').forEach(function(esp) {
          var e = esp.trim();
          if (e) { var o=document.createElement('option'); o.value=e; o.textContent=e; sel.appendChild(o); }
        });
      }
    ...
    if (lbl1) lbl1.textContent = (tipo && tipo.label_contagem) ? tipo.label_contagem : 'Nº de pessoas';

    var cont2wrap = document.getElementById('res-cont2-wrap');
    var lbl2 = document.getElementById('res-lbl-cont2');
    if (tipo && tipo.label_contagem2) {
      if (cont2wrap) cont2wrap.style.display = 'block';
      if (lbl2) lbl2.textContent = tipo.label_contagem2;
    } else {
  ...
  function openEditar(idOrEl) {
    ...
    if (r.espaco) {
      ...
      var achou = false;
      if (espSel) {
        for (var oi=0; oi<espSel.options.length; oi++) {
          if (espSel.options[oi].value === r.espaco) { espSel.value = r.espaco; achou = true; break; }
        }
      }
      if (!achou && espSel) {
