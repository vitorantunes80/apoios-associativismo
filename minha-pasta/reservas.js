...
var Reservas = (function() {
  ...
  function _expandirRecorrentes(reservas, dias) {
    ...
    reservas.forEach(function(r) {
      if (!r.recorrente) {
        // reserva simples — mostrar se cai nesta semana
        var ri = new Date(r.data_inicio);
        dias.forEach(function(d) {
          if (ri.toDateString() === d.toDateString()) {
            eventos.push({ reserva: r, data: d, hora: ri.getHours() });
          }
        });
        return;
      }
    ...
    HORAS.forEach(function(hStr, hi) {
      ...
      dias.forEach(function(d, di) {
        ...
        eventos.forEach(function(ev) {
          ...
          var dur = 1; // horas
          if (r.recorrente && r.hora_inicio && r.hora_fim) {
            var hi2 = parseInt(r.hora_inicio.split(':')[0],10);
            var hf2 = parseInt(r.hora_fim.split(':')[0],10);
            dur = Math.max(1, hf2 - hi2);
          } else if (!r.recorrente && r.data_inicio && r.data_fim) {
            dur = Math.max(1, (new Date(r.data_fim) - new Date(r.data_inicio)) / 3600000);
          }

          var height = dur * 26 - 3;
          html += '<div class="sched-event" style="background:' + est.bg + ';color:' + est.cor + ';border-left:3px solid ' + est.cor + ';height:' + height + 'px;z-index:1;" onclick="event.stopPropagation();Reservas.openEditar(\'' + r.id + '\')">';
          ...
          if (r.recorrente) html += '<div style="font-size:9px;opacity:.6;">🔁 recorrente</div>';
          if (r.estado === 'pre_reserva') html += '<div style="font-size:9px;font-weight:700;">PRÉ-RESERVA</div>';
          html += '</div>';
        });
  ...
  function getModaisHTML() {
    ...
    html += '<div class="form-row"><div class="fi"><label class="fl">Data e hora início *</label><input type="datetime-local" id="res-inicio" class="fin"></div>';
    html += '<div class="fi"><label class="fl">Data e hora fim *</label><input type="datetime-local" id="res-fim" class="fin"></div></div>';
    html += '</div>';
  ...
  function clickCelula(el, eqId) {
    ...
    if (!hora || !data) return;
    var h2 = String(parseInt(hora)+1).padStart(2,'0');
    openNova(eqId, 'confirmada', data + 'T' + String(hora).padStart(2,'0') + ':00', data + 'T' + h2 + ':00');
  }

  function openNova(eqId, estado, inicio, fim) {
    APP.editId = null;
    ...
    if (inicio) document.getElementById('res-inicio').value = inicio;
    if (fim)    document.getElementById('res-fim').value    = fim;
  ...
  function salvar() {
    ...
    if (isRec) {
      ...
      var di = document.getElementById('res-inicio').value;
      var df = document.getElementById('res-fim').value;
      if (!di) { toast('A data de início é obrigatória.', 'error'); return; }
      if (!df) { toast('A data de fim é obrigatória.', 'error'); return; }
      if (new Date(di) >= new Date(df)) { toast('O fim deve ser posterior ao início.', 'error'); return; }
      body.data_inicio = new Date(di).toISOString();
      body.data_fim    = new Date(df).toISOString();
    }
  ...
  return {
    ...
    onTipoChange: onTipoChange, onDiaChange: onDiaChange
  };
