...
var Utilizadores = (function() {
  ...
  function render() {
    ...
    if (isAdmin) {
      html += '<button class="btn btn-primary" onclick="Utilizadores.openNovo()">+ Novo utilizador</button>';
    }
    html += '</div>';

    if (!APP.utilizadores.length) {
      return html + emptyState('Sem utilizadores', 'Nenhum utilizador registado.');
    }

    // Separar DDS de Equipamentos
    var dds = APP.utilizadores.filter(function(u) { return u.isDds; });
    var eqs = APP.utilizadores.filter(function(u) { return !u.isDds; });
    ...
    utilizadores.forEach(function(u) {
      ...
      if (isAdmin) {
        ...
        if (APP.user && u.id !== APP.user.id) {
          html += '<button class="btn btn-ghost btn-sm" style="color:var(--red);" onclick="Utilizadores.confirmarApagar(\'' + u.id + '\',\'' + H(u.nome) + '\')">Apagar</button>';
        }
        html += '</div></td>';
      }
      html += '</tr>';
  ...
  function getModaisHTML() {
    ...
    html += '<div class="mh-ic mhi-red"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6"/></svg></div>';
    html += '<div><div class="modal-title">Apagar utilizador</div><div class="modal-sub" id="m-util-ap-nome">—</div></div>';
    html += '<button class="modal-close" onclick="closeModal(\'m-util-apagar\')">✕</button>';
    html += '</div>';
    html += '<div class="modal-body"><p style="font-size:13.5px;color:var(--text-2);line-height:1.6;">Esta acção é irreversível. O utilizador perderá acesso à plataforma.</p></div>';
    html += '<div class="modal-footer">';
    html += '<button class="btn btn-ghost" onclick="closeModal(\'m-util-apagar\')">Cancelar</button>';
    html += '<button class="btn btn-danger" onclick="Utilizadores.execApagar()">Apagar utilizador</button>';
    html += '</div></div></div>';
