...
var Escalas = (function() {
  ...
  function _bloquearEdicao(bloquear) {
    ['esc-func','esc-tipo','esc-di','esc-df','esc-he','esc-hs','esc-funcao','esc-obs'].forEach(function(id) {
      var el = document.getElementById(id);
      if (el) el.disabled = !!bloquear;
    });
    // esconder também a secção de repetição
    ...
    if (footer && bloquear) {
      footer.innerHTML = '<button class="btn btn-primary" onclick="closeModal(\'m-escala-novo\')">Fechar</button>';
    }
  }
  ...
  function openEditar(id) {
    ...
    // — folgas/férias são geridas nos respectivos módulos
    var soConsulta = !APP.podeGerirEscalas(e.equipamento_id)
                  || e.tipo_turno === 'folga' || e.tipo_turno === 'ferias';
    if (soConsulta) _bloquearEdicao(true);
  ...
  function salvar() {
    ...
    listaDatas.forEach(function(d) {
      blocos.forEach(function(b) {
        ...
        t.data_inicio  = d;
        t.data_fim     = datas.length ? d : df.value;
        t.hora_entrada = b.he;
        t.hora_saida   = b.hs;
        lote.push(t);
      });
  ...
  function _loadEscalas(cb) {
    ...
    .then(function(data) {
      ...
      // FUNDIR com o que já existe (não substituir!) — senão o dashboard
      // e os outros equipamentos ficavam sem turnos
      var base = (APP.escalas || []).filter(function(e) {
        return !novos.some(function(n) { return n.id === e.id; });
      });
