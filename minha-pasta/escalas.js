...
var Escalas = (function() {
  ...
  function render(eqId) {
    ...
    // ── DIAGNÓSTICO TEMPORÁRIO (v...s) — remover depois ──
    html += '<div id="esc-diag" style="background:#FEF3C7;border:1px solid #F59E0B;border-radius:8px;padding:10px;margin:8px 0;font-size:11px;font-family:monospace;color:#92400E;line-height:1.6;word-break:break-all;">A testar o pedido ao servidor…</div>';
    (function(eqId) {
      var dias = _getWeekDates();
      var ini = new Date(dias[0]); ini.setDate(ini.getDate() - 14);
      var fim = new Date(dias[6]); fim.setDate(fim.getDate() + 14);
      var di = ini.toISOString().split('T')[0];
      ...
      // Pedido A: exactamente o que a app faz (filtro combinado)
      var qA = 'equipamento_id=eq.' + eqId + '&data_inicio=gte.' + di + '&data_fim=lte.' + dfim + '&order=data_inicio.asc';
      // Pedido B: só por equipamento, sem filtro de datas
      var qB = 'equipamento_id=eq.' + eqId;

      sbGet('escalas', qA).then(function(a) {
        var nA = Array.isArray(a) ? a.length : ('ERRO: ' + JSON.stringify(a));
        sbGet('escalas', qB).then(function(b) {
          var nB = Array.isArray(b) ? b.length : ('ERRO: ' + JSON.stringify(b));
          var ex = (Array.isArray(b) && b[0]) ? b[0] : null;
          pinta('<b>DIAGNÓSTICO DO PEDIDO</b><br>' +
            'A (filtro app: eq + datas) → ' + nA + '<br>' +
            'B (só equipamento) → ' + nB + '<br>' +
            'janela de datas: ' + di + ' a ' + dfim + '<br>' +
            (ex ? ('exemplo B: di=' + JSON.stringify(ex.data_inicio) + ' df=' + JSON.stringify(ex.data_fim)) : 'B sem exemplo'));
        }).catch(function(e) { pinta('B falhou: ' + (e && (e.message||JSON.stringify(e)))); });
      }).catch(function(e) { pinta('A falhou: ' + (e && (e.message||JSON.stringify(e)))); });
    })(eq.id);
