...
var Horas = (function() {
  ...
  function onTipoChange() {
    ...
    if (mot) mot.placeholder = gozo ? 'ex: Assuntos pessoais' : 'ex: Apoio ao torneio de sábado';

    var circ = document.getElementById('h-circuito');
    if (circ) {
      circ.innerHTML = gozo
        ? '<strong>Circuito de aprovação:</strong> o pedido segue para o responsável do equipamento e, depois, para o chefe da sua unidade. Depois de autorizado, é lançado na sua escala e descontado do saldo.'
        : '<strong>Circuito de aprovação:</strong> o pedido segue para o responsável do equipamento e, depois de este validar, para o chefe da sua unidade. Só conta no saldo depois de aprovado.';
    }
  ...
  function salvar() {
    ...
    sbPost('banco_horas', body).then(function(resp) {
      ...
      // gozo já aprovado (chefe/admin) → lançar logo na escala
      if (body.estado === 'aprovado') _lancarNaEscala(criado || body);
  ...
  function _notificarResponsavel(r, func) {
    ...
    destinos.forEach(function(uid) {
      ...
      sbPost('avisos', {
        ...
        mensagem: (func ? func.nome : 'Um funcionário') + ' pediu autorização de ' + _fmtH(r.horas) + ' (' + tipoTxt + ') em ' + _fdata(r.data) + (perTxt ? ' — ' + perTxt : '') + '. Aguarda a sua autorização.',
        lido: false,
        tipo: 'notificacao',
