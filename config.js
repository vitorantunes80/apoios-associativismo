...
// Credenciais demo → mapeadas para username na tabela utilizadores
var DEMO_PASS = {
  admin: 'admin',
  resp_dds: 'resp',
  sup_udaj: 'sup',
...
function loadAll(cb) {
  Promise.all([
    ...
    sbGet('funcionarios', 'order=nome.asc').then(function(d) {
      APP.funcionarios = Array.isArray(d) ? d : [];
    }).catch(function() {
      APP.funcionarios = []; // tabela pode não existir ainda
    })
  ]).then(function() {
    if (cb) cb(null);
  }).catch(function(err) {
